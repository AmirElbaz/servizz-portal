...
export default function DashboardVariantA({ projects, featuredProject, heroStats }: Props) {
  ...
  useEffect(() => {
    ...
    function transition(next: HeroState) {
      ...
      const lockTime = next === "hidden" ? 650 : 600;
        ...
        >
          ...
          >
            ...
            {isCompact ? (
              ...
              <div className="relative z-10 w-full px-6 sm:px-10 lg:px-16 py-8 sm:py-14 lg:py-20">
                <div className="flex items-start justify-between gap-4 sm:gap-10">
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.25em] text-white/30 mb-4">
                      {featuredProject.code} &middot; Featured Portal
                    </p>
                    <h1 className="text-3xl sm:text-5xl lg:text-7xl font-black tracking-tighter font-headline leading-[0.9] text-white mb-5">
                      {featuredProject.name}
                    </h1>
                    <p className="text-white/40 max-w-lg leading-relaxed text-sm sm:text-base lg:text-lg hidden sm:block">
                      {featuredProject.fullDescription}
                    </p>
                  </div>
                  <div className="shrink-0 bg-white rounded-2xl p-3 sm:p-4 hidden sm:block">
                    <img src={featuredProject.logo} alt={featuredProject.name} className="h-16 sm:h-20 lg:h-28 object-contain" />
                  </div>
                </div>

                <div className="flex items-end justify-between gap-4 sm:gap-8 mt-8 sm:mt-12">
      ...
      <main className={`pb-16 px-6 lg:px-12 max-w-7xl mx-auto ${isHidden ? "pt-28" : "pt-6"}`}>
        <section>
          <div ref={projectsRef} className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold font-headline text-on-surface tracking-tight">Servizz.gov</h2>
            <div className="flex items-center gap-1 bg-surface-container-high/60 rounded-xl p-1">
              <button onClick={() => setViewMode("grid")} className={`p-1.5 rounded-lg transition-colors ${viewMode === "grid" ? "bg-white shadow-sm text-primary" : "text-on-surface-variant hover:text-on-surface"}`}>
                <span className="material-symbols-outlined text-[20px]">grid_view</span>
              </button>
              <button onClick={() => setViewMode("list")} className={`p-1.5 rounded-lg transition-colors ${viewMode === "list" ? "bg-white shadow-sm text-primary" : "text-on-surface-variant hover:text-on-surface"}`}>
          ...
          {viewMode === "grid" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {projects.map((project, i) => (
                <Link key={project.id} to={`/project/${project.id}`}
                  className={`group relative bg-white rounded-2xl p-6 no-underline card-lift overflow-hidden border border-transparent hover:border-on-surface-variant/10 ${isHidden ? "animate-card-rise" : ""}`}
                  style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)", animationDelay: isHidden ? `${i * 70}ms` : undefined }}>
            ...
            <div className="space-y-2">
              {projects.map((project, i) => (
                <Link key={project.id} to={`/project/${project.id}`}
                  className={`group relative bg-white rounded-2xl p-5 no-underline card-lift overflow-hidden border border-transparent hover:border-on-surface-variant/10 flex items-center gap-5 ${isHidden ? "animate-card-rise" : ""}`}
                  style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)", animationDelay: isHidden ? `${i * 55}ms` : undefined }}>
